            ...
            if surveys_in:
                ...
                if not from_scratch:
                    ...
                    if course_sessions:
                        ...
                        past_sessions = list(course_sessions).index(next_session)
                    else:
                        past_sessions = 0
                for cohort in course_cohorts:
                    ...
                    if not from_scratch:
                        try:
                            ...
                            last_computed_session = eval(last_computed_results)[-1][0]
                        except (Script.DoesNotExist, NameError, IndexError):
                            last_computed_session = 0
                        if past_sessions and last_computed_session:
                            low_index = len(course_cohorts) * last_computed_session * 2 - 1
                    ...
                    if surveys_in:
                        ...
                        if len(survey_data) >= 1:
                            if (not from_scratch) and last_computed_session:
                                ...
                                for i in range(0, len(survey_data)):
                                    if survey_data[i].count(None) < len(survey_data[i]):
                                        ...
                                        survey_data[i].insert(0, last_computed_session + i + 1)
